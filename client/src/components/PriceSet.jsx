...
import { setPriceRangeSet } from "../store/slices/diamondFiltersSlices";
...
const Price = () => {
  const priceRangeSet = useSelector(state => state.diamondFilters.filters.priceRangeSet)
  const dispatch = useDispatch();

  const [minVal, setMinVal] = useState(priceRangeSet[0]);
  const [maxVal, setMaxVal] = useState(priceRangeSet[1]);

  const updateRedux = (min, max) => {
    dispatch(setPriceRangeSet([min, max]));
  };

  useEffect(() => {
    setMinVal(priceRangeSet[0]);
    setMaxVal(priceRangeSet[1]);
  }, [priceRangeSet])
